function makeLineChart(dataset, xName, yObjs, axisLables) {
  ...
  let getYScaleFn = (yObj) =>  (d) => chartObj.yScale(yObjs[yObj].yFunct(d));

  for (let yObj in yObjs) {
    yObjs[yObj].line = d3.svg.line().interpolate("cardinal").x( (d) =>  {
  ...
  chartObj.render = function () {
    ...
    // Year label
    focus.append("text").attr("class", "focus humity").attr("x", 9).attr("y", 7);
    // Focus line
    ...
    function mousemove() {
      ...
      focus.select(".focus.humity").text("Humedad: " + chartObj.xFormatter(chartObj.xFunct(d))+ '%');
    }
...
d3.csv('assets/airpadel/template1/grip_chart.csv',  (error, data) => {
  data.forEach((d) => {
    d.humedad = +d.humedad;
